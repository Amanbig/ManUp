  ...
  try {
    ...
    if (caller.type !== 'owner' && caller.type !== 'admin') {
      ...
      if (memberRecord.length === 0) {
        return res
          .status(403)
          .json({ detail: 'Only project or organization admins can create environments' });
      }
    }
  ...
  try {
    ...
    if (caller.type !== 'owner' && caller.type !== 'admin') {
      ...
      if (memberRecord.length === 0) {
        return res
          .status(403)
          .json({ detail: 'Only project or organization admins can update this environment' });
      }
    }
  ...
  try {
    ...
    if (caller.type !== 'owner' && caller.type !== 'admin') {
      ...
      if (memberRecord.length === 0) {
        return res
          .status(403)
          .json({ detail: 'Only project or organization admins can delete this environment' });
      }
    }
