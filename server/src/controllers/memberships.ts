  ...
  try {
    ...
    if (caller.type !== 'owner' && caller.type !== 'admin') {
      ...
      if (projectMembership.length === 0) {
        return res
          .status(403)
          .json({ detail: 'Only project/organization owners or admins can manage members' });
      }
    }
...
export const deleteProjectMember = async (req: Request, res: Response) => {
  try {
    ...
    const deleted = await database
      ...
      .where(
        and(eq(projectMembers.project_id as any, id), eq(projectMembers.user_id as any, userId)),
      )
      .returning();
    ...
    if (caller.type !== 'owner' && caller.type !== 'admin') {
      ...
      if (projectMembership.length === 0) {
        return res.status(403).json({
          detail: 'Only project/organization owners or admins can manage environment access',
        });
      }
    }
