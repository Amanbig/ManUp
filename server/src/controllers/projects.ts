...
export const getProject = async (req: Request, res: Response) => {
  try {
    ...
    const projectRecord = await database
      ...
      .from(projects)
      .where(
        and(eq(projects.id as any, id), eq(projects.organization_id as any, user.organizationId)),
      )
      .limit(1);
  ...
  try {
    ...
    if (caller.type !== 'owner' && caller.type !== 'admin') {
      ...
      if (memberRecord.length === 0) {
        return res
          .status(403)
          .json({ detail: 'Only project or organization admins can update this project' });
      }
    }
  ...
  try {
    ...
    if (caller.type !== 'owner' && caller.type !== 'admin') {
      ...
      if (memberRecord.length === 0) {
        return res
          .status(403)
          .json({ detail: 'Only project or organization admins can delete this project' });
      }
    }
