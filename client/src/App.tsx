        ...
        <div className="flex h-screen bg-[#0b0b0f] text-neutral-100 overflow-hidden font-sans">
            ...
            <aside
                className={`fixed md:relative inset-y-0 left-0 z-40 bg-[#0e0e13] border-r border-neutral-900 flex flex-col shrink-0 overflow-hidden transition-transform md:transition-[width] duration-200 ease-in-out ${
                    isSidebarOpen ? "translate-x-0 w-72 md:w-72" : "-translate-x-full md:translate-x-0 w-20 md:w-20"
                }`}
            >
                {/* Brand / Logo */}
                <div className={`h-16 border-b border-neutral-900 flex items-center shrink-0 transition-all duration-200 ${isSidebarOpen ? "justify-between px-4 w-72" : "justify-center w-20"}`}>
                    {isSidebarOpen ? (
                        <>
                ...
                {/* Switchers Section */}
                <div className={`border-b border-neutral-900 flex flex-col shrink-0 overflow-hidden transition-all duration-200 ${isSidebarOpen ? "p-4 space-y-4 w-72" : "p-2 space-y-2 w-20 items-center"}`}>
                    {isSidebarOpen ? (
                        <>
                        ...
                        <>
                            {/* Collapsed views */}
                            <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-neutral-950 border border-neutral-900 text-orange-400 cursor-default" title={`Organization: ${currentOrg?.name || ""}`}>
                                <Building2 className="h-5 w-5" />
                            </div>
                            <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-neutral-950 border border-neutral-900 text-orange-400 cursor-default" title={`Active Project: ${selectedProject?.name || ""}`}>
                ...
                {/* Sidebar Navigation */}
                <nav className={`flex-1 transition-all duration-200 ${isSidebarOpen ? "p-4 space-y-1 w-72" : "p-2 space-y-2 w-20"}`}>
                    <button
                        onClick={() => setActiveTab("secrets")}
                        title={!isSidebarOpen ? "Secrets Vault" : undefined}
                        className={`${
                            isSidebarOpen ? "w-full flex items-center gap-3 px-3 py-2.5" : "h-10 w-10 flex items-center justify-center mx-auto"
                ...
                {/* User footer & Logout */}
                <div className={`border-t border-neutral-900 bg-neutral-950/40 flex items-center transition-all duration-200 shrink-0 ${isSidebarOpen ? "p-4 justify-between w-72" : "p-2 justify-center w-20"}`}>
                    {isSidebarOpen && (
